...
function send_player_spawn(_steam_id, _slot) {
	...
	var _b = pkt_write(NETWORK_PACKETS.SPAWN_SELF, pkt_schema(NETWORK_PACKETS.SPAWN_SELF), _pos);
	steam_net_packet_send(_steam_id, _b);
	buffer_delete(_b);
...
function send_other_player_spawn(_steam_id, _pos) {
	var _b = pkt_write(NETWORK_PACKETS.SPAWN_OTHER, pkt_schema(NETWORK_PACKETS.SPAWN_OTHER), {
		x:       _pos.x,
		y:       _pos.y,
		steamID: _steam_id,
	});
...
function send_player_input_to_clients(_player_input){
	...
	var _b = pkt_write(NETWORK_PACKETS.SERVER_PLAYER_INPUT, pkt_schema(NETWORK_PACKETS.SERVER_PLAYER_INPUT), _player_input);
